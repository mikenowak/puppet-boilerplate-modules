...
describe 'boilerplate', :type => 'class' do
  ...
  let(:node) { 'puppetdev.hiveworx.co.uk' }
  ...
  describe 'Test for invalid parameters' do
    ...
    debianish.each do |os|
      describe "for operating system #{os}" do
        ...
        it { expect { should contain_package("#{debianish_package}").with_ensure('present') }.to raise_error(Puppet::Error, /"#{params[:ensure]}" is not a valid ensure parameter value/) }
      end
    end
    ...
    redhatish.each do |os|
      describe "for operating system #{os}" do
        ...
        it { expect { should contain_package("#{redhatish_package}").with_ensure('present') }.to raise_error(Puppet::Error, /"#{params[:ensure]}" is not a valid ensure parameter value/) }
      end
    end
